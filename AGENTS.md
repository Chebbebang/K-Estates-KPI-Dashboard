...
- `npm run dev` — starts API server (:3001) and Vite dev server (:5173, proxies `/api` → :3001) concurrently
- `npm run build` — production build via Vite (outputs to `dist/`)
- `npm start` — production: Node serves `dist/` + API on :3001
...
- **`src/`** — React 18 + Vite frontend. Polls `GET /api/kpis` every 60s via `src/hooks/useKpiData.js` (also refetches on window focus). `src/App.jsx` defines the KPI card configs; `src/components/KpiCard.jsx` renders a card (shows "no access" state when `ok:false`).
...
- `tasks.task.list` fails with `401 insufficient_scope` for this webhook — the Overdue Tasks card intentionally degrades to "no access".
